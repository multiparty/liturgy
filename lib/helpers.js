...
let writeOutput = function(jiffInstance, payload, outputFileName, cfg) {

    let outputPath = `${cfg.outputPath}/${outputFileName}.csv`;
    fs.writeFile(
        outputPath,
        formatOutput(payload, cfg.protocol.header),
        function (err) {
            if (err) {
                console.log(err);
            }
